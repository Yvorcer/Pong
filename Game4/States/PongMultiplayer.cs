...
namespace Game4
{
    ...
    {
        Texture2D background, hearthSprite, paddleSPR;
        List<Paddle> paddles = new List<Paddle>(); // Creating a list of Paddles for easy Drawing and Updating.
        Ball ball;
        public bool pause;

        public PongMultiplayer(ContentManager Content)
        {
            background = Content.Load<Texture2D>("Background");
            hearthSprite = Content.Load<Texture2D>("Hearth");
            paddleSPR = Content.Load<Texture2D>("Bat");

            paddles.Add(new Paddle(Content, 15, (int)Pong.Screen.Y / 2, Keys.W, Keys.S, Color.Blue));
            paddles.Add(new Paddle(Content, ((int)Pong.Screen.X / 4), (int)Pong.Screen.Y / 2, Keys.F, Keys.V, Color.LightBlue));
            paddles.Add(new Paddle(Content, (int)Pong.Screen.X - 15, (int)Pong.Screen.Y / 2, Keys.Up, Keys.Down, Color.Red));
            paddles.Add(new Paddle(Content, ((int)Pong.Screen.X / 4) * 3, (int)Pong.Screen.Y / 2, Keys.K, Keys.M, Color.Salmon));
        ...
        public void Update(GameTime gameTime)
        {
            if (!pause)
            {
                foreach (var paddle in paddles) // Going over each paddle to see if one Collides with the ball.
                {
                    OnCollission(paddle, gameTime);
                    paddle.Update(gameTime, (int)Pong.Screen.Y);
                }
        ...
        public void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            {
                paddle.Draw(gameTime, spriteBatch, paddle.color);
            }
        ...
        private void OnCollission(Paddle paddles, GameTime gametime)
        {
            if (ball.collisionBox.Intersects(paddles.collisionBox))
            {
                Pong.PaddleHit.Play();

                float width = paddleSPR.Height / 2;
                float offset = paddles.paddlePosition.Y - ball.ballPosition.Y;
                ...
                while (ball.collisionBox.Intersects(paddles.collisionBox))
                {
        ...
        public void Reset()
        {
            ...
            foreach (var paddle in paddles) // Going over each paddle and placing it back.
            {
                paddle.Reset();
            }
